...
浏览器只加载构建后的 Web 资源，不直接读取 NBT、mod jar 或 Minecraft 资源包。当前使用 Three.js 方块几何体展示结构，并加载 AE2 的真实基础纹理；完整模型、逐面材质、发光层和方块状态仍待后续实现。
...
### 7.4 实现最小资源到模型流程（AE2 基础纹理已完成）

- 已根据标准化方块 ID 为 `test.nbt` 中的 AE2 方块选择基础纹理。
- 已将固定纹理整理到仓库，构建时生成浏览器可以加载的场景数据和纹理。
- 待处理模型继承、逐面贴图、基础旋转、发光层和静态预览图。

产物：一个包含场景 JSON 和纹理的 `mc-generated` 场景目录；模型与预览图后续补充。
